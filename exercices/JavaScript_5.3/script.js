...
// filhos/filhas da tag <ul> com ID "days" . Note que os dias 29 e 30 de novembro estão no array pois
// representam respectivamente Domingo e Segunda-feira.
...
function addDaysList(array, ulMain, holidays, fridays) {
  for (let i = 0; i < array.length; i += 1) {
    day = document.createElement('li');
    day.className = 'day';
    if (holidays.includes(array[i])) day.className += ' holiday';
    if (fridays.includes(array[i])) day.className += ' friday';
    day.innerHTML = array[i];
    ulMain.appendChild(day);
  }
}

addDaysList(dezDaysList, days, holidays, fridays);
...
function addBtn(string, father, id) {
...
addBtn('Feriados', buttonsContainer, 'btn-holiday');
...
function eventAdd(element, eventType, elementFunction, ...paramentros) {
  element.addEventListener(eventType, (e) =>
    elementFunction(e, ...paramentros)
  );
}
...
// Exercício 4:
// Implemente uma função que receba como parâmetro a string "Sexta-feira" e crie dinamicamente um

...
// aqui além de reduce será necessário utilizar também a função map . Dica: Você pode acessar o index do array
...
const studentAverage = () => students.reduce((acc, crr, index) => {
  const average = grades[index].reduce((total, currency) => total + currency) / grades[index].length
  const obj = {name: crr, average}
  acc.push(obj)
  return acc  
}, []) 

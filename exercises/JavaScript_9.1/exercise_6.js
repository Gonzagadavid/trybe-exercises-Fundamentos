...
// muito ocupado com outras operações. Logo, adicione na função sendMarsTemperature um outro callback que contenha
...
const sendMarsTemperature = (callback, error) => {
  ...
  setTimeout(() => (time < 3000 ? callback(getMarsTemperature()) : error('Error getting temperature: Robot is busy')), time);
};
